...
╠➥ About 送信者について
╠✪〘 Settings 〙✪═══
╠➥ AutoAdd On/Off 自動参加するをあく
╠➥ AutoLeave On 自動参加するを閉める
╠➥ AutoRead On/Off 自動既讀
╠➥ Prompt On/Off 状態通知
╠➥ ReRead On/Off メッセしたを撤回したこと調べる
╠➥ Pro On/Of 全部を守る
╠➥ Protect On/Off キックアウト保護
╠➥ QrProtect On/Off ネット保護
╠➥ Invprotect On/Off 誘って保護
╠➥ Getinfo On/Off 友達狀況
╠➥ Detect On/Off タッグ検出
╠✪〘 Self 〙✪═════
╠➥ Me 我的連結
...
if "" not in ban["owners"]:
    ban["owners"].append("ud12fc3f07ae3edba145942264da801fd")
#==============================================================================#
def lineBot(op):
    try:
        if op.type == 0:
        ...
        if op.type == 5:
            if settings["autoAdd"] == True:
                ...
                sendMention(op.param1, " @! 感謝您加我為好友w",[op.param1])
        if op.type == 11:
        ...
        if op.type == 13:
            if clMID in op.param3:
                ...
                if op.param2 in ban["admin"] or op.param2 in ban["owners"]:
                    ...
                    sendMention(op.param1, "感謝權限者 @! 邀請入群",[op.param2])
                else:
                    cl.acceptGroupInvitation(op.param1)
                ...
                if gInviMids == []:
                    ...
                    cl.sendMessage(op.param1,"請求拒絕\n此用戶在黑名單內\n如需解黑單請聯繫作者")
                    cl.sendMessage(msg.to, None, contentMetadata={'mid':'u582a84400dbb26662be4ad3da63d68af'}, contentType=13)
        if op.type == 17:
            ...
            if op.param1 in settings["mention"]:
                ...
                sendMention(op.param1, "歡迎女裝大佬 @! 加入"+name,[op.param2])
        if op.type == 19:
            if op.param1 in settings["mention"]:
                chiya=[op.param2]
                chiya.append(op.param3)
                sendMention(op.param1,"[提示]\n@! 把 @! 拖出去處理了OAO", chiya)
            if op.param2 in ban["admin"] or op.param2 in ban["bots"] or op.param2 in ban["owners"]:
                pass
...
#==============================================================================#
            if sender in ban["owners"]:
                if text.lower() == 'restart':
                    cl.sendMessage(to, "重啟成功，正在登入")
                    restartBot()
                if text.lower() == 'rs':
                    cl.sendMessage(to, "重啟成功，數秒後即可繼續使用")
                    restartBot()
                    ...
                    if inkey in ban["admin"]:
                        ...
                        json.dump(ban, codecs.open('bot/ban.json','w','utf-8'), sort_keys=True, indent=4, ensure_ascii=False)
                elif msg.text.lower().startswith("owneradd ") or msg.text.lower().startswith("oadd "):
                    MENTION = eval(msg.contentMetadata['MENTION'])
                    inkey = MENTION['MENTIONEES'][0]['M']
                    if inkey not in ban["owner"] and inkey not in ban["blacklist"] and inkey not in ban["admin"]: 
                        ban["owner"].append(str(inkey))
                        cl.sendMessage(to, "已獲得權限！")
                        json.dump(ban, codecs.open('bot/ban.json','w','utf-8'), sort_keys=True, indent=4, ensure_ascii=False)
                elif msg.text.lower().startswith("ownerdel ") or msg.text.lower().startswith("odel "):
                    MENTION = eval(msg.contentMetadata['MENTION'])
                    inkey = MENTION['MENTIONEES'][0]['M']
                    if inkey in ban["owner"]:
                        ban["owner"].remove(str(inkey))
                        cl.sendMessage(to, "已取消權限！")
                        json.dump(ban, codecs.open('bot/ban.json','w','utf-8'), sort_keys=True, indent=4, ensure_ascii=False)
